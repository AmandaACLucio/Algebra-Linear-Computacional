...
def polynomial_quadrature(count_points_integration, point_a, point_b):
    ...
    zpoints, weights = select_polynomial_quadrature(count_points_integration, point_a, point_b)
    return sum([function_task5(zpoints[i])*weights[i] for i in range(count_points_integration)])